...
> **Note on the data:** the dataset accompanying this repository is a synthetic sample constructed to mirror realistic SaaS subscription behavior. It demonstrates the analytical workflow rather than describing a real company.
...
4. **Explore in Excel** - overall churn, monthly trend, plan, billing cycle, company size, channel, churn reasons and behavioral comparison, used as an independent check on the SQL layer.
5. **Analyze in SQL** - segmentation, ranking and revenue-loss quantification with minimum sample-size guards.
...
[`sql/saas_churn_revenue_analysis.sql`](sql/saas_churn_revenue_analysis.sql) is organized as numbered business questions rather than a syntax showcase:
...
| 9 | Which active customers should Customer Success contact first? | Risk segmentation and prioritized worklist |
...
1. **Prioritize Starter onboarding and adoption.** Starter carries the highest churn rate and the largest customer counts. Test a structured first-30-day onboarding path and measure whether feature adoption at day 30 moves Starter retention.
...
Language here is deliberate: **test, investigate, monitor, review.** The data supports prioritization, not causal claims.
